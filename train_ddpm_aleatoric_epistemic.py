    ...
    for epoch in range(args.n_epochs):
        ...
        for step, batch in progress_bar:
            ...
            with autocast(enabled=True):
                ...
                # this is the prediction before the output head that is assumed to be fixed and does not need to be computed multiple times for epistemic computation
                h = diffusion.module.forward_backbone(x=noisy_image, timesteps=timesteps, context=None) if NUM_GPUS > 1 else diffusion.forward_backbone(x=noisy_image, timesteps=timesteps, context=None)
                ...
                pred_mean_final = pred_means.mean(dim=0)
                ...
                loss = args.diff_loss_weight * heteroscedastic_loss(pred_mean_final, logvar_total, noise)

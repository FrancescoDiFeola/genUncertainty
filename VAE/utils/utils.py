...
def tb_display_reconstruction_3D(writer, step, image, recon, label):
    """
    Display 3D volume reconstruction in TensorBoard during AE training.
    
    Args:
        writer (SummaryWriter): TensorBoard writer.
        step (int): Training step for logging.
        image (Tensor): Original 3D volume (C, D, H, W or D, H, W).
        recon (Tensor): Reconstructed 3D volume (C, D, H, W or D, H, W).
        label (str): Unique identifier (e.g. "PET", "CT", etc.).
    """
    import matplotlib.pyplot as plt

    plt.style.use('dark_background')
    fig, ax = plt.subplots(nrows=3, ncols=3, figsize=(10, 8))  # 3 slices per volume

    for _ax in ax.flatten():
        _ax.set_axis_off()

    # If (C, D, H, W), squeeze out channel dimension
    if image.ndim == 4:
        image = image[0]
    if recon.ndim == 4:
        recon = recon[0]
    ...
    # Plot original
    ax[0, 0].set_title('Original - Axial', color='cyan')
    ax[0, 0].imshow(image[mid_slices['axial'], :, :], cmap='gray')

    ax[0, 1].set_title('Original - Coronal', color='cyan')
    ax[0, 1].imshow(image[:, mid_slices['coronal'], :], cmap='gray')

    ax[0, 2].set_title('Original - Sagittal', color='cyan')
    ax[0, 2].imshow(image[:, :, mid_slices['sagittal']], cmap='gray')

    # Plot recon
    ax[1, 0].set_title('Reconstructed - Axial', color='magenta')
    ax[1, 0].imshow(recon[mid_slices['axial'], :, :], cmap='gray')

    ax[1, 1].set_title('Reconstructed - Coronal', color='magenta')
    ax[1, 1].imshow(recon[:, mid_slices['coronal'], :], cmap='gray')

    ax[1, 2].set_title('Reconstructed - Sagittal', color='magenta')
    ax[1, 2].imshow(recon[:, :, mid_slices['sagittal']], cmap='gray')

    # Plot difference
    ax[2, 0].set_title('Difference - Axial', color='yellow')
    ax[2, 0].imshow(diff[mid_slices['axial'], :, :], cmap='gray')

    ax[2, 1].set_title('Difference - Coronal', color='yellow')
    ax[2, 1].imshow(diff[:, mid_slices['coronal'], :], cmap='gray')

    ax[2, 2].set_title('Difference - Sagittal', color='yellow')
    ax[2, 2].imshow(diff[:, :, mid_slices['sagittal']], cmap='gray')

    plt.tight_layout()
    writer.add_figure(f"3D_Reconstruction_{label}", fig, global_step=step)
    plt.close(fig)

...
def sample_and_plot_batch_ddim_aleatoric(
    ...
    for t in progress:
        ...
        with autocast(enabled=True):
            ...
            pred_noise = refiner1(pred_noise, pred_logvar)
            pred_logvar = refiner1(pred_logvar, pred_noise)
...
if __name__ == '__main__':
    ...
    print(diffusion)
    error_refiner = networks.FiLMRefiner().to(DEVICE)
    variance_refiner = networks.FiLMRefiner().to(DEVICE)

    if NUM_GPUS > 1:
        print(f"Using {NUM_GPUS} GPUs")
        diffusion = torch.nn.DataParallel(diffusion)
        error_refiner = torch.nn.DataParallel(error_refiner)
        variance_refiner = torch.nn.DataParallel(variance_refiner)

    optimizer = torch.optim.AdamW(list(diffusion.parameters()) + list(error_refiner.parameters()) + list(variance_refiner.parameters()), lr=args.lr)
    ...
    for epoch in range(args.n_epochs):
        ...
        for step, batch in progress_bar:
            ...
            with autocast(enabled=True):
                ...
                pred_mean = error_refiner(pred_mean_var[0], pred_mean_var[1])
                pred_var = variance_refiner(pred_mean_var[1], pred_mean)

                # Compute loss
                loss = args.diff_loss_weight * heteroscedastic_loss(pred_mean, pred_var, noise)
            ...
            if step % 150 == 0:
                sample_and_plot_batch_ddim_aleatoric(
                    diffusion_model=diffusion,
                    error_refiner=error_refiner,
                    variance_refiner=variance_refiner,
        ...
        if epoch % 50 == 0:
            ...
            torch.save(attention_refiner.state_dict(), os.path.join(args.output_dir, f'attention_refiner-ep-{epoch}.pth'))

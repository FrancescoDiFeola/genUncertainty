...
class LDCTHDCTAutoKLDataset():
    ...
    def __getitem__(self, index):
        ...
        if index % 2 == 0:
            # Pick from domain 1
            img_path = self.annotations_A['img_path'].iloc[index // 2]
            label = 0  # Assign a label for domain 1
        else:
            # Pick from domain 2
            img_path = self.annotations_B['img_path'].iloc[index // 2]
            label = 1  # Assign a label for domain 2

        img_raw = pydicom.dcmread(img_path, force=True)  # Read the DICOM file
        img = self.transforms(img_raw)  # Apply transformations

        return {'A': img, 'B': img, 'label': label, 'A_paths': img_path, 'B_paths': img_path}

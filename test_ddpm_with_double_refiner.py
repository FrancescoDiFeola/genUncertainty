...
def run_inference_and_log(
    ...
    for t in tqdm(scheduler.timesteps, desc="DDIM Sampling"):
        ...
        pred_logvar = refiner2(pred_logvar, pred_noise)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--refiner1_ckpt', type=str, required=True)
    parser.add_argument('--refiner2_ckpt', type=str, required=True)
    ...
    error_refiner = networks.init_refiner(args.refiner1_ckpt).to(DEVICE)
    variance_refiner = networks.init_refiner(args.refiner2_ckpt).to(DEVICE)
    ...
    csv_path = os.path.join(args.output_dir, f"{args.experiment_name}_metrics_epoch_550.csv")
    ...
    with open(csv_path, mode='w', newline='') as csvfile:
        ...
        for step, batch in enumerate(loader):
            run_inference_and_log(
                ...
                refiner1=error_refiner,
                refiner2=variance_refiner,

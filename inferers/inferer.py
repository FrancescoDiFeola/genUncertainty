...
class DiffusionInferer(Inferer):
    ...
    def __call__(
        self,
        inputs: torch.Tensor,
        concat: torch.Tensor,  # aggiungo un input per garantire la concatenazione anche quando si fa cross-attention
        ...
        noisy_image = torch.cat([noisy_image, concat], dim=1)
        if mode == "concat":

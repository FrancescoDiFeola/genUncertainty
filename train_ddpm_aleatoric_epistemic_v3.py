...
if __name__ == '__main__':
    ...
    for epoch in range(args.n_epochs):
        ...
        for step, batch in progress_bar:
            ...
            # First: backprop main_loss only (retain computation graph for reuse)
            ...
            # Enable gradients only for epistemic head
            if NUM_GPUS > 1:
                epistemic_head = diffusion.module.epistemic_head
            else:
                epistemic_head = diffusion.epistemic_head

            for param in epistemic_head.parameters():
                param.requires_grad = True

            # Then: backprop alignment_loss only for epistemic head
            scaler.scale(alignment_loss).backward()
            ...
            # Unfreeze all parameters for next step
            for param in diffusion.parameters():
                param.requires_grad = True            
            # Logging
            writer.add_scalar('train/loss', main_loss.item(), global_counter['train'])
            ...
            epoch_loss += (main_loss+alignment_loss).item()

...
def index_slices_by_subject(data_path, modalities=["t1n", "t1c", "t2w", "t2f"], min_idx=0, max_idx=999):
    valid_modalities = {"t1n", "t1c", "t2w", "t2f", "CT", "PET"}
...
class CTPETDataset(Dataset):
    def __init__(self, opt):
        ...
        assert len(self.modalities) >= 2, \
            "You must specify at least two modalities (e.g., ['t1n', 't2w'])"
        ...
        if opt.under_sample_dataset:
            self.samples = [x for i, x in enumerate(self.samples) if (i + 1) % 3 == 1]  # CTPET (i + 1) % 3 != 0
        ...
        print(f"Modalities sampled uniformly: {self.modalities}")
    ...
    def __getitem__(self, idx):
        ...
        # Randomly select modality
        modality = random.choice(self.modalities)

        img_path = self.subject_dict[subject][modality][slice_idx]

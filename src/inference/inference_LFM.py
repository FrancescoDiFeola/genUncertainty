...
import torch
import os
from src.inference.utils import sparsification_curve_fast, random_sparsification_fast, sparsification_curve, random_sparsification, norm_percentile, collect_calibration_data, map_correlations_multi_thresholds, map_correlations, summarize_uncertainty, uncertainty_error_tail_bins_torch
...
def run_inference_LFM_self_refining_and_log_uncertainty_propagation(
    ...
    #####
    # gt_z = autoencoder.encode(gt_batch)
    # gt = autoencoder.decode(gt_z)
    # gt = gt.cpu().detach()
    #####
...
####################### Latent Rectified Flow Matching self-refining #########################
@torch.no_grad()
def run_inference_LFM_self_refining_and_log_uncertainty_eval(
        diffusion_model,
        autoencoder,
        context_encoder,
        writer,
...
@torch.no_grad()
def run_inference_LFM_self_refining_and_log_uncertainty_calibration_tail_bins(
        diffusion_model,
        autoencoder,
        context_encoder,
        writer,
...
@torch.no_grad()
def run_inference_LFM_self_refining_and_log_uncertainty_propagation_ablation(
        diffusion_model,
        autoencoder,
        context_encoder,
        writer,
    ...
    for i, (t, next_t) in enumerate(tqdm(zip(scheduler.timesteps, all_next_timesteps), total = min(len(scheduler.timesteps), len(all_next_timesteps)),)):
        ...
        if i == 0:
            ...
            context_vector = context_encoder(torch.zeros_like(uncertainty_map))
...
@torch.no_grad()
def run_inference_LFM_vanilla_and_log_MC_sampling_uncertainty_eval(
        diffusion_model,
        autoencoder,
        condition_batch,
        gt_batch,
...
@torch.no_grad()
def run_inference_LFM_vanilla_and_log_MC_sampling_uncertainty_calibration_tail_bins(
        diffusion_model,
        autoencoder,
        condition_batch,
        gt_batch,
